...
void gbemg_playhandler_fn(void* content) /* this thread is  charged for play or stop play emg info*/
{
	...
	case GBEMG_MSG_eActArrial:
	{
		...
		if(ishighplaypriority(procparam.pcaseindex->emg_case_urgent_level))
		{
			setplaymask(procparam.pcaseindex->emg_case_urgent_level,procparam.pcaseindex->emg_case_id);
		...
		if(curplayemgid==procparam.pcaseindex->emg_case_id)
			{
				...
				clearplaymask(procparam.pcaseindex->emg_case_urgent_level);
