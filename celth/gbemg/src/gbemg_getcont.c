...
GBEMG_CaseContent_t* GetContFromParam(U16 contPid,U16 Ebm_id)
{
	...
	if(hf)
	{
		...
		/* table id mask*/
		contFilterSettings.mask[0]=0xFF;

		/* ebm id mask*/
		contFilterSettings.mask[2]=0xFF;
		contFilterSettings.mask[3]=0xFF;
	
		/* section num mask*/
		contFilterSettings.mask[5]=0xFF;
	...
	ret=CELTH_WaitForEvent(ev,10000);
...
/* begin search other section of content table*/
				...
				/* we dont check if there are  section, just get it*/ 
					CelthApi_Debug_Msg("\nGet the %x num section of cont, infact num is[%x],",ii,sectbuf[6]);
